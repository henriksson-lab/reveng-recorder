...
#[command(
    name = "reveng-viewer",
    version,
    about = "Session timeline / traffic inspector"
)]
struct Cli {
    ...
    for (i, v) in track.iter().enumerate() {
        match v {
            Some(v) => {
                ...
                if let Some(pp) = prev {
                    ui.painter()
                        .line_segment([pp, p], egui::Stroke::new(1.5, color));
                }
...
impl App {
    fn new(mut model: SessionModel, track_control: Option<String>) -> Self {
        ...
        let (track, track_label) = match &track_control {
            Some(name) => (
                model::normalize_track(&model.value_track(name)),
                name.clone(),
            ),
    ...
    fn reload(&mut self, ctx: &egui::Context) {
        ...
        self.rows = self
            .model
            .frames_around(&ckpt, self.window)
            .unwrap_or_default();
...
impl eframe::App for App {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        egui::Panel::top("timeline").show(ui, |ui| {
            ...
            // same time axis.
            draw_density_strip(ui, &self.density, 18.0, |s| {
                egui::Color32::from_rgb(40, s, s)
            });
            draw_density_strip(ui, &self.density_pcie, 12.0, |s| {
                egui::Color32::from_rgb(s, 40, s)
            });
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for row in &self.rows {
                            ...
                            if !row.hex.is_empty() {
                                ui.label(
                                    egui::RichText::new(format!("    {}", row.hex))
                                        .monospace()
                                        .weak(),
                                );
                            }
